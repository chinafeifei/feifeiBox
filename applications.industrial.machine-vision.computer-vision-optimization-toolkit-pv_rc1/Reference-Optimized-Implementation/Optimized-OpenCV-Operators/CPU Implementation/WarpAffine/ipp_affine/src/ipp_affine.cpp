...
int main(int argc, char *argv[])
{
    ...
    if (resolution[0] != 0 && resolution[1] != 0) {
        data = ippsMalloc_8u(resolution[0] * resolution[1] * channels);
        memset(data, IMG_COLOR, resolution[0] * resolution[1] * channels); //creat a picture with some color, for example gray
        Mat fakeImg(resolution[1], resolution[0], CV_8UC1, data);
        printf("\nresolution: %d * %d, channels: %d, IPP fake image\n", fakeImg.cols, fakeImg.rows, fakeImg.channels());
        pSrc = fakeImg.data;
        srcSize = {fakeImg.cols, fakeImg.rows};
        dstSize = {fakeImg.cols, fakeImg.rows};
        srcStep = fakeImg.cols * fakeImg.channels();
        dstStep = fakeImg.cols * fakeImg.channels();
        pDst = ippsMalloc_8u(dstSize.width * dstSize.height * fakeImg.channels());
