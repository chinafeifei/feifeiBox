...
void compare(Ipp16s *single_dst, Ipp16s *multi_dst, cv::Mat &cv_dst)
{
    ...
    for (int x = 0; x < cv_dst.rows; x++)
    {
        ...
        for (int y = 0; y < cv_dst.cols; y++)
        {
            ...
            if (single_dst[x * cv_dst.cols + y] != cv_dst.at<short>(x, y))
            {
...
void sobel_ipp_single_thread(cv::Mat &img, Ipp16s *dst)
{
    std::cout << endl
              << endl
              << "=============  SINGLE thread sobel abs sum ==================" << endl;
    ...
    status = ippiFilterSobelHorizBorder_8u16s_C1R(pSrc, step8, hDst, step16,
                                                  roiSize, ippMskSize3x3, borderType, 0, pBuffer);
    displayArray(hDst, img.rows, img.cols);

    log("ipp single vertical sobel");
    status = ippiFilterSobelVertBorder_8u16s_C1R(pSrc, step8, vDst, step16,
                                                 roiSize, ippMskSize3x3, borderType, 0, pBuffer);
    displayArray(vDst, img.rows, img.cols);

    log("ipp single horizon absolute");
    status = ippiAbs_16s_C1R(hDst, step16, hAbsDst, step16, roiSize);
    displayArray(hAbsDst, img.rows, img.cols);

    log("ipp single vertical absolute");
    status = ippiAbs_16s_C1R(vDst, step16, vAbsDst, step16, roiSize);
    displayArray(vAbsDst, img.rows, img.cols);

    log("ipp single 2 abs add");
    status = ippiAdd_16s_C1RSfs(hAbsDst, step16, vAbsDst, step16, dst, step16, roiSize, 0);
    ...
    delete[] hAbsDst; // horizon  square result
    delete[] vAbsDst; // vertical square result
    ippsFree(pBuffer);
}
...
void sobel_ipp_multi_thread(cv::Mat &img, Ipp16s *dst)
{
    ...
    std::cout << endl
              << endl
              << "=============  MULTI thread sobel abs sum ==================" << endl;
    ...
    log("ipp multi thread horizon absolute");
    statusT = ippiAbs_16s_C1R(hDst, step16, hAbsDst, step16, roiSize);
    displayArray(hAbsDst, img.rows, img.cols);

    log("ipp multi thread vertical absolute");
    statusT = ippiAbs_16s_C1R(vDst, step16, vAbsDst, step16, roiSize);
    displayArray(vAbsDst, img.rows, img.cols);

    log("horizon abs + vertical abs");
    statusT = ippiAdd_16s_C1RSfs_LT(hAbsDst, step16, vAbsDst, step16, dst, step16, roiSizeL, 0 /* scale */);
    displayArray(dst, img.rows, img.cols);
...
void sobel_opencv(cv::Mat &img, cv::Mat &dst)
{
    ...
    // abs_grad_x.convertTo(abs_grad_x, CV_16S);
    // .convertTo(abs_grad_y, CV_16S);
    ...
    if (DEBUG)
    {
        ...
        cout << "grad_y = \n"
             ...
             << endl;
        cout << "abs_grad_x = \n"
             << format(abs_grad_x, Formatter::FMT_PYTHON) << ";" << endl
             << endl
             << endl;
        cout << "abs_grad_y = \n"
             << format(abs_grad_y, Formatter::FMT_PYTHON) << ";" << endl
             << endl
             << endl;
        cout << "dst = \n"
