...
// openMP raw_version
void getMinMaxValue_openMP_thread_chunk(vector<Point> &, int, int, int, float *);
...
// openMP Version3 without dynamic
void getMinMaxValue_openMP3_thread(vector<Point> &, int, int, int, float *);
...
int main()
{
	...
	// To decide the matrix length. DO NOT bigger than 3
	vector<Point> matrixAll;
	for (int i = 0; i < VECTOR_LENGTH; i++)
	{
		matrixAll.insert(matrixAll.end(), matrixAll_read.begin(), matrixAll_read.end());
	}
	Sleep(2 * 1000);
	...
	// test_fun(FOR_TIME, NUM_THREAD, CHUNK, matrixAll, getMinMaxValue_openMP3_thread);
	// Sleep(2 * 1000);
...
void getMinMaxValue_openMP_thread_chunk(vector<Point> &mat, int rows, int thread, int chunk, float ans[6])
{
	float max_x_element = mat[0].x;
	float max_y_element = mat[0].y;
	float max_z_element = mat[0].z;
...
void getMinMaxValue_openMP3_thread(vector<Point> &mat, int rows, int thread, int chunk, float ans[6])
{
	float max_x_element = mat[0].x;
	float max_y_element = mat[0].y;
	float max_z_element = mat[0].z;
...
#pragma omp parallel for num_threads(thread) reduction(max                                                          \
