...
#define USING_ACCUMULATE_COLSUM true
...

{
    ...
    printf("\n TBB & own IPP Time consuming(kernel size %d): %f\n", 11, time_tbb / (LOOP_NUM));
    compare((Ipp8u *)cvtImage2.m_ptr, smoothed, img);
}
// void ipp_meanfilter(cv::Mat img, cv::Mat smoothed)
// {
//     IppiSize maskSize = {11, 11};
//     int iTmpBufSize = 0;
...
//     status = ippiFilterBoxBorderGetBufferSize(roiSize, maskSize, ipp8u, 1, &iTmpBufSize);

//     int srcStep = img.cols * sizeof(Ipp8u), dstStep = img.cols * sizeof(Ipp8u);
//     ;
//     Ipp8u *pSrc = NULL;
...
//     auto ipp_time = std::chrono::duration_cast<timeunit>(ipp_stop - ipp_start).count() / 1000.0 / 200;
//     printf("\n Original IPP Time consuming(kernel size %d): %f\n", 11, ipp_time);
//     compare(pDst, smoothed, dstStep);
