...
void sobel_opencv_hypot(cv::Mat &src, float scale, cv::Mat &dst)
{
    ...
    cv::Mat square_x, square_y, add_dst;
    ...
    std::cout << "The sobel hypot elapsed time in opencv was " << elapsed / num_iters << " ms" << std::endl;
}
...
int main(int argc, char **argv)
{
    ...
    // opencv sobel
    sobel_opencv_hypot(srcMat, imageScale, dstMat);

    // dpcpp sobel
    apply_sobel_hypot(dimage_dst, srcMat.data, imageScale, width, height, thd_per_blk);
    ...
    // diff(dstMat, dstMat_dpcpp, width, height);
    ...
    free(himage_dst);
    free(dimage_dst);
    cv::imwrite("./data/dstMat_hypot.ppm", dstMat);
    cv::imwrite("./data/dstMat_dpcpp_hypot.ppm", dstMat_dpcpp);
