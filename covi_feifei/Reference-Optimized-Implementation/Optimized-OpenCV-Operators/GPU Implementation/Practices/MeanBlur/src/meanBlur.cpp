...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    for (int i = 0; i < num_iters; i++)
    {
        ...
        int ksize = 11;
        cv::boxFilter(srcMat, dstMat, -1, cv::Size(ksize, ksize), cv::Point(-1, -1), true, cv::BORDER_REPLICATE);
        auto end = get_time_point();
        time_opencv += get_duration(start, end);
    }
    std::cout << "meanBlur CPU: " << time_opencv / num_iters << "ms\n";

    // Erosion with kernel size 3*3
    // ErosionEsimd(srcMat.data, dstMat_dpcpp.data, width, height, ratio);
    // Erosion with kernel size 5*5
    // meanBlurEsimd3x3(srcMat.data, dstMat_dpcpp.data, width, height, ratio);
    // meanBlurEsimd5x5(srcMat.data, dstMat_dpcpp.data, width, height, ratio);
    meanBlurEsimd11x11(srcMat.data, dstMat_dpcpp.data, width, height, ratio);
