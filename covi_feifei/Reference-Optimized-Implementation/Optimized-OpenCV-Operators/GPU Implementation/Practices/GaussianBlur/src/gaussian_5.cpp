    ...
    */
    ...
    auto e = q.submit([&](handler &cgh) {
      ...
      cgh.parallel_for<class Gaussian>(
        ...
        simd<uchar, (BLKH+4)*BLKW*2> inA_v;
        simd<float, (BLKH+4)*BLKW> mX_v;
        simd<float, BLKW*BLKH> mX_out_v;
        simd<uchar, BLKW*BLKH> outX_v;

        simd<float, (BLKH+4)*BLKW*2> inA_vf;

        auto inA = inA_v.bit_cast_view<uchar, BLKH+4, BLKW*2>();
        auto inAf = inA_vf.bit_cast_view<float, BLKH+4, BLKW*2>();
        ...
        inA_vf = convert<float, int, BLKW*2*(BLKH+4)>(inA_v);

        mX = (Coeffs0 * inAf.select<BLKH+4,1,BLKW,1>(0,0))
            + (Coeffs1 * inAf.select<BLKH+4,1,BLKW,1>(0,1))
            + (Coeffs2 * inAf.select<BLKH+4,1,BLKW,1>(0,2))
            + (Coeffs3 * inAf.select<BLKH+4,1,BLKW,1>(0,3))
            + (Coeffs4 * inAf.select<BLKH+4,1,BLKW,1>(0,4));
...
int main(void) {
  ...
  */
  ...
  cv::imwrite("./data/gaussian_5.ppm", cvdst);
  cv::imwrite("./data/gaussian_5_dpcpp.ppm", esimdDst);
