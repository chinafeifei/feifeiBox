                                   ...
                                   uint32_t local_size, uint32_t num_blocks)
{
  ...
  for (int y = 0; y < num_blocks; y++)
  {
    ...
    /* bit_cast_view<EltTy>( ): returns a reference to the calling simd object
     * interpreted as a new simd vector with the size determined by the template
     * EltTy parameter.
     */
...
int main(int argc, char **argv)
{
   ...
   * Reports errors using SYCL exception classes. The value returned must be
   * equal to that returned by get_info<info::queue::context>()
   */
