...
ESIMD_INLINE simd<T, 64> transpose_matrix(simd<T, 64> v1) {
  ...
  simd_mask<16> mask = 0;
  mask.select<8, 2>(0) = 1;
  /* bit_cast_view<EltTy, Height, Width>( ): returns a reference to the calling
   * simd object interpreted as a new 2D simd_view object with the shape
   * determined by the template parameters Height andWidth. The size of the new
   * 2D block must not exceed the size of the original object.
   */
...
bool runTest(unsigned MZ, unsigned block_size) {
  ...
  initMatrix(M, MZ); /*MZ * MZ square matrix */
